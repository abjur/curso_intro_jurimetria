...
3. **Encontrando o repertório "ideal".** Você deve balancear a parcimônia x precisão. As palavras-chave devem ser as mais genéricas e simples possíveis, a fim de não deixar nenhum processo de fora, mas elas devem também ser as mais precisas possíveis para que a listagem de processos não tenha um monte de "lixo". 
    - É impossível de encontrar as palavras-chaves perfeitas, que vão capturar 100% dos processos de interesse, sem pegar "lixo". Na dúvida, sempre opte por escolher palavras mais genéricas do que mais restritas.
...
1. **Como esse preenchimento é feito?** A partir de uma lista fechada, o advogado escolhe qualquer classe e assunto, podendo inclusive colocar classes e assuntos mais "genéricos" do que o possível.
...
tibble::tibble(
  ...
  `Técnicas de computação` = c("Scraper", "Scraper e tratamento de texto", "Scraper", "Não precisa")
) |> 
  knitr::kable(
    format = "html", escape = FALSE,
    table.attr = "class=\"striped\""
...
:::: columns
::: {.column width="60%"}
<iframe src="img/dje-resultados-1.svg" width="810" height="540"></iframe>
:::
::: {.column width="40%"}
...
- Muitas vezes, em bases de processos muito grandes, a melhor opção acaba sendo analisar apenas uma parte da base. Para isso, precisamos nos valer de técnicas de _amostragem_ 
...
- **Escopo.** Recomendamos também que exista uma pergunta no formulário sobre escopo.
  - É por causa da possibilidade de se criar esta pergunta que sugerimos, na listagem de processos, que se use palavras mais genéricas, do que palavras mais restritas. É possível classificar "limpar" 
...
É preciso ter algum software para analisar os dados. Na ABJ utilizamos muito o R. Mas é possível também utilizar outros sofwtares. A seguir, listamos alguns softwares comuns.
