...
__Viés de seleção__ é um tipo de viés estatístico, indicando uma diferença entre os __indivíduos que estamos estudando na base de dados__ de dados e __a população que desejamos estudar__.

Coloquialmente, viés de seleção é uma forma de dizer que estamos __tirando conclusões sobre bananas ao estudar maçãs__. 
...
Queremos estudar a __letalidade__ da Covid-19. Para isso, precisamos calcular a razão entre a quantidade de mortes e a quantidade de pessoas infectadas.
...
O mecanismo de seleção, nesse caso, é a __entrada no hospital__: uma pessoa só entra na base de dados se for para o hospital.
...
Nesse caso, os aviões que caíram __foram atingidos em outros lugares__, diferentes da ponta das asas. Ou seja, os engenheiros protegeram a parte menos importante do avião!

O mecanismo de seleção, nesse caso, é a __queda do avião__: o avião só entra na base de dados se sobreviver à guerra.
...
Se as partes tem um nível de informação (medido pela variância das expectativas) parecido, os valores de $P_p$ e $P_d$ são, em média, iguais. 
...
O artigo assume que um conflito possui um __nível de mérito__ $y$, desconhecido, que vem de uma distribuição de probabilidades também desconhecida. 
