...
**Neste exercício, queremos treinar uma habilidade essencial da realização de qualquer pesquisa em jurimetria: a listagem de processos. As questões a seguir servem para guiar as alunas e os alunos no processo de listagem.**
...
**2) A partir do tema escolhido, encontre os processos referentes a ele no CJPG (Consulta de Julgados de Primeiro Grau), utilizando a Pesquisa Livre, Classe e Assunto (não é preciso utilizar todos os campos, apenas aqueles que o grupo julgar necessário). Anotem os termos pesquisados, as classes e os assuntos e respondam:**

**a) Todos os processos encontrados estão dentro do escopo?**
...
**a) A partir da pesquisa realizada no CJPG na questão 2, responda quais são as informações que você tem de cada processo? Isto é, para cada um dos resultados que a busca gerou, sem acessar as decisões, quais informações estão disponíveis?**
...
**b) Pegue um processo qualquer que você encontrou no CJPG e coloque o número deste processo no CPOPG (Consulta de Processos de Primeiro Grau). Quais informações por processo estão disponíveis neste sistema sem acessar os autos? Compare essas informações com aquelas encontradas no CJPG. Existem informações que existiam no CJPG e que não existem no CPOPG? E o contrário? O que é igual e o que é diferente entre cada sistema, quando às informações disponíveis?**
...
**c) É possível responder às perguntas de pesquisa do exercício 1 com as meta informações disponíveis nos sistemas? Indique quais perguntas podem ser respondidas apenas com as meta-informações e, se houver, quais não podem ser respondidas dessa forma.**
...
**4) Quanto ao escopo, responda:**

**a) A pesquisa é retrospectiva ou prospectiva? Por quê?**

Retrospectiva, pois usamos o CJPG para listar os processos. 

**b) As perguntas de pesquisa estão adequadas ao escopo? Explique.**

As perguntas referentes ao tempo de resposta não estão adequadas, pois uma pesquisa retrospectiva introduz viés de seleção no tempo de duração dos processos. 
